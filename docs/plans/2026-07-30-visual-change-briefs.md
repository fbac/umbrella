...
**A known, deliberate limit.** Neither the shell block nor the canary detects an assertion rewritten to compare a value to itself — `chk(m, r.offsite, r.offsite)`. The source count is unchanged, every selector still resolves, `chk` still genuinely compares, and the assertion still executes, so the runtime inventory counts it. Measured when this suite held 38 assertions, which is why these three numbers do not line up with Step 4's current ones: three assertions so rewritten leave a clean run at `38 passed, 0 failed` and degrade the gutted-`secure`-list control from `35 passed, 3 failed` to `36 passed, 2 failed`. The shape is what carries forward, not the totals — detection erodes rather than vanishing, which is why it is tolerated. Closing it needs per-call-site pinning of every expected value, which was weighed and declined: it would churn on every ordinary edit for a mutation nothing plausible produces by accident.
...
bash assets/change-brief/tests/render_test.sh; echo "exit=$?"
```
Expected, measured: `shell: 339 passed, 0 failed`, `exit=0`.
...
- Gutting the mermaid `secure` list to `["secure","securityLevel","startOnLoad"]` gives `37 passed, 3 failed`: `beacon: zero offsite requests` collects four live requests to `evil.example.invalid`, `directive cannot re-enable HTML labels` counts 6 `<foreignObject>`, and `directive cannot inject CSS into the emitted stylesheet` counts 2 payload-bearing `<style>` elements.
- Deleting `nsIds(slot, id);` gives `39 passed, 1 failed`: `themes: no id is shared between the light and the dark render` reports nine duplicated ids — `arrowhead`, `crosshead`, `sequencenumber`, `filled-head`, `clock`, `database`, `computer`, and the flowchart's two edge paths `L-T1-T2-0` and `L-T1-T3-0` — while `markersOnPage` holds at 8, so the failure is the duplication and not a page that lost its markers.
- Dropping `overflow-wrap:anywhere` from the `.doc` rule gives `39 passed, 1 failed`: `narrow: an unbreakable token in prose never scrolls the body at 480px` measures `documentElement.scrollWidth` 632 against a `clientWidth` of 480, at the top of the page and again at the bottom, with `tokenWiderThanViewport` still true — the probe page really does hold a token the viewport cannot fit.
- Injecting a `setTimeout(() => __definitely_not_defined__(), 0)` gives `35 passed, 5 failed`, one per page-error assertion.
- Removing the `h-` prefix from `slug()` now gives `40 passed, 0 failed` — the mutant SURVIVES, where it gave `37 passed, 1 failed` with `deadAnchors` reporting `["database"]` before `nsIds()` existed. `nsIds()` renames the diagram's `database` `<symbol>` to `mmd-0-database`, so `plan-region.md`'s `## Database` heading no longer collides with anything and the prefix is no longer what keeps that index entry resolving. Confirmed by stacking the two mutations: with `nsIds(slot, id);` deleted *and* the prefix removed, `deadAnchors` reports `["database"]` again. `deadAnchors` still covers the property it names — an index entry resolving to something other than its own heading, from any id in any future bundle — and the prefix line itself is still pinned verbatim by the shell suite, so it cannot be deleted quietly. What is gone is the behavioural discrimination: no fixture in this harness now fails when the prefix does.

These are Task 13, Task 14 and Task 15 properties that had never been executed in a browser before this task, plus the two that a real browser walk of the rendered dogfood brief found after it: neither the duplicate ids nor the sideways scroll is visible to the shell suite, to this harness as it stood, or to jsdom, because one is a layout measurement and the other is which of two same-named elements a `url(#)` reference resolves to.
