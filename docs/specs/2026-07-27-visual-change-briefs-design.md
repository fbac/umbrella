...
| `scan.awk` fabricates "unterminated fence" warnings on intact documents (a backtick info string containing a backtick is a paragraph to `marked`, a fence to the scanner) | A reader-facing banner cries wolf on a correct brief, which round four established disarms the backstop. **This is why the requirement "the page raises no banner when the document is intact" is currently unmet** — the requirement states the target, this row states the gap. | Suppress diag banners when the sentinel check passes; keep the stderr line for the author. The plan must carry this fix and that requirement as **one task**, or the assertion gets written against current behavior instead of intended behavior. |
