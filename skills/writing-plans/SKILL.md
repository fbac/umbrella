...
Write comprehensive implementation plans assuming the engineer has zero context for our codebase and questionable taste. Document everything they need to know: which files to touch for each task, code, testing, docs they might need to check, how to test it. Give them the whole plan as bite-sized tasks. DRY. YAGNI. Minimal tests. Frequent commits.
...
- "Write the one test that proves the contract behavior" - step
- "Break the change, run the test, confirm it fails; restore, confirm it passes" - step
- "Commit" - step
...
- [ ] **Step 2: Write the test that proves the contract behavior**
...
Expected: PASS. Then break the change in Step 1, rerun, and expect FAIL on the
assertion. Restore the change.
...
- DRY, YAGNI, minimal tests (one per contract behavior, real calls over mocks), frequent commits
