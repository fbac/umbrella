...
#           "screen_dir":"/path/to/project/.umbrella/brainstorm/12345-1706000000/content",
...
**Note:** Pass the project root as `--project-dir` so mockups persist in `.umbrella/brainstorm/` and survive server restarts. Without it, files go to `/tmp` and get cleaned up. Remind the user to add `.umbrella/` to `.gitignore` if it's not already there.
...
If the session used `--project-dir`, mockup files persist in `.umbrella/brainstorm/` for later reference. Only `/tmp` sessions get deleted on stop.
